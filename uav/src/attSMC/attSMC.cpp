...
attSMC::attSMC(const LayoutPosition *position, string name)
    : ControlLaw(position->getLayout(), name,3) {
  pimpl_ = new attSMC_impl(this, position, name);
  
  SetIsReady(true);
}

attSMC::~attSMC(void) { delete pimpl_; }

void attSMC::UseDefaultPlot(const gui::LayoutPosition *position) {
  pimpl_->UseDefaultPlot(position);
}

void attSMC::Reset(void) {
  pimpl_->i = 0;
  pimpl_->first_update = true;
}

float attSMC::GetIntegral(void) const { return pimpl_->i; }

void attSMC::UpdateFrom(const io_data *data) {
  pimpl_->UpdateFrom(data);
  ProcessUpdate(output);
}


void attSMC::SetValues(float q0,        float q1,        float q2,        float q3,
